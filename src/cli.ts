...
import { BomBuilder, BomBuilderOptions, OmittableDependencyTypes } from './bom-builder';
...
interface CommandOptions {
  ...
  omit: Set<OmittableDependencyTypes>;
  ...
  return new Command()
    ...
    .addOption(
      new Option(
        ...
        .choices(Object.values(Omittable))
        .default(
          process.env.NODE_ENV === "production" ? [Omittable.Dev] : [],
    ...
    .addOption(
      new Option(
        ...
        .choices(Object.keys(Spec.SpecVersionDict))
    ...
    .addOption(
      (function () {
        ...
        const oldParseArg =
          o.parseArg ?? // might do input validation on choices, etc...
          ((v) => v); // fallback
    ...
    .addOption(
      ...
      ).default(OutputStdOut, "write to STDOUT")
    )
    .addOption(
      new Option("--mc-type <type>", "Type of the main component.")
        .choices(
...
export function run(process: NodeJS.Process): void {
  ...
  const myConsole: Console = new console.Console(
    process.stderr,
    process.stderr
  );
  ...
  switch (options.outputFormat) {
    case OutputFormat.XML:
      serializer = new Serialize.XmlSerializer(
        new Serialize.XML.Normalize.Factory(spec)
      );
      break;
    case OutputFormat.JSON:
      serializer = new Serialize.JsonSerializer(
        new Serialize.JSON.Normalize.Factory(spec)
      );
      break;
  }
